...
from configs.default.PFINet_strategy import strategy_cfg
...
class PFINet(nn.Module):
    def __init__(self, cfg,feature_dim = 5888,num_class=395):
        ...
        if self.cfg.Learning_W_RGB:
            self.w_RGB = nn.Parameter(torch.randn(size=(1,3,384,128)))
    def forward(self, x, ids,cam_ids):
        ...
        feat1 = self.avg_pool(feat1_att).view(feat_final1.size(0),-1)
        feat2 = self.avg_pool(feat2_att).view(feat_final2.size(0), -1)
        feat3 = self.avg_pool(feat3_att).view(up_feat4.size(0), -1)
        feat4 = self.avg_pool(feat4).view(feat4.size(0), -1)
        feat4_ = self.avg_pool(feat4_att).view(feat4_att.size(0), -1)
