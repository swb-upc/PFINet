...
class RGB_to_Infrared(nn.Module):
    def __init__(self, cfg,feature_dim = 5888,num_class=395):
        ...
        #space attention
        self.space_att = SpaceAttention(2048)
    ...
    def forward(self, x, ids,cam_ids):
        ...
        #space attention
        feat4_att = self.space_att(feat4)


        feat1 = self.avg_pool(feat_final1).view(up_feat2.size(0),-1)
        feat2 = self.avg_pool(feat_final2).view(up_feat3.size(0), -1)
        feat3 = self.avg_pool(feat_final3).view(up_feat4.size(0), -1)
        feat4 = self.avg_pool(feat4).view(feat4.size(0), -1)
        feat4_ = self.avg_pool(feat4_att).view(feat4_att.size(0), -1)
        feat_pool = torch.cat([feat4_,feat4,feat3,feat2,feat1],dim=-1)
