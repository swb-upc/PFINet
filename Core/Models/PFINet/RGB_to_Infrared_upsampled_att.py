...
class RGB_to_Infrared(nn.Module):
    def __init__(self, cfg,feature_dim = 5888,num_class=395):
        ...
        #space attention
        self.space_att = nn.Conv2d(in_channels=3840,out_channels=1,kernel_size=1)
    ...
    def forward(self, x, ids,cam_ids):
        ...
        feat1_down_sampled = F.adaptive_avg_pool2d(feat_final1,output_size=(24,8))
        feat2_down_sampled = F.adaptive_avg_pool2d(feat_final2,output_size=(24,8))

        feat = torch.cat([feat1_down_sampled,feat2_down_sampled,feat_final3,feat4],dim=1)
        #space attention
        mask = self.activation(self.space_att(feat))
        feat = feat * mask
        feat_pool = self.avg_pool(feat).view(feat.size(0),-1)
        feat_pool1 = self.avg_pool(feat4).view(feat4.size(0),-1)

        bn_feat = self.bottleneck(torch.cat([feat_pool1,feat_pool],dim=-1))
