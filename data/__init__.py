...
def get_train_loader(dataset, root, sample_method, batch_size, p_size, k_size, image_size, random_flip=False, random_crop=False,
                     random_erase=False, color_jitter=False, padding=0, num_workers=4,RGB_to_Infrared=False,R_channel=False):
    # data pre-processing
    ...
    # dataset
    if dataset == 'sysu':
        train_dataset = SYSUDataset(root, mode='train', transform=transform,RGB_to_Infrared=RGB_to_Infrared,R_channel=R_channel)
    elif dataset == 'regdb':
        train_dataset = RegDBDataset(root, mode='train', transform=transform,RGB_to_Infrared=RGB_to_Infrared,R_channel=R_channel)
    elif dataset == 'market':
...
def get_test_loader(dataset, root, batch_size, image_size, num_workers=4,RGB_to_Infrared=False,R_channel=False):
    # transform
    transform = T.Compose([
        T.Resize(image_size),
        T.ToTensor(),
    ...
    # dataset
    if dataset == 'sysu':
        gallery_dataset = SYSUDataset(root, mode='gallery', transform=transform,RGB_to_Infrared=RGB_to_Infrared,R_channel=R_channel)
        query_dataset = SYSUDataset(root, mode='query', transform=transform,RGB_to_Infrared=RGB_to_Infrared,R_channel=R_channel)
    elif dataset == 'regdb':
        gallery_dataset = RegDBDataset(root, mode='gallery', transform=transform,RGB_to_Infrared=RGB_to_Infrared,R_channel=R_channel)
        query_dataset = RegDBDataset(root, mode='query', transform=transform,RGB_to_Infrared=RGB_to_Infrared,R_channel=R_channel)
    elif dataset == 'market':
