...
def get_test_loader(dataset, root, batch_size, image_size, num_workers=4,RGB_to_Infrared=False):
    # transform
    transform = T.Compose([
        T.Resize(image_size),
        T.ToTensor(),
    ...
    # dataset
    if dataset == 'sysu':
        gallery_dataset = SYSUDataset(root, mode='gallery', transform=transform,RGB_to_Infrared=RGB_to_Infrared)
        query_dataset = SYSUDataset(root, mode='query', transform=transform,RGB_to_Infrared=RGB_to_Infrared)
    elif dataset == 'regdb':
