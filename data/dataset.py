...
class SYSUDataset(Dataset):
    def __init__(self, root, mode='train', transform=None,RGB_to_Infrared=False,R_channel=False):
        assert os.path.isdir(root)
        assert mode in ['train', 'gallery', 'query']
        self.RGB_to_Infrared = RGB_to_Infrared
        ...
        print('====RGB_Infrared>', self.RGB_to_Infrared)
        print('====R_chaneel>', self.R_channel)
    def __len__(self):
    ...
    def __getitem__(self, item):
        ...
        if self.RGB_to_Infrared:
            if self.cam_ids[item] in [1,2,4,5]:
                img = np.array(img.convert('L'))
                img = np.expand_dims(img,axis=0).repeat(repeats=3,axis=0).transpose(1,2,0)
                # img = np.concatenate([np.array([img]),np.array([img]),np.array([img])])
                img = Image.fromarray(np.uint8(img))
                # print("RGB_to_Infrared jpg save")
                # img.save('1.jpg')
            ...
            if self.cam_ids[item] in [1,2,4,5]:
                ...
                img = np.expand_dims(img,axis=0).repeat(repeats=3,axis=0).transpose(1,2,0)
                # img = np.concatenate([np.array([img[0]]),np.array([img[0]]),np.array([img[0]])])
                # print(img)
                img = Image.fromarray(np.uint8(img))
                # print("R_channel jpg save")
                # img.save('1.jpg')
...
class RegDBDataset(Dataset):
    def __init__(self, root, mode='train', transform=None,RGB_to_Infrared=None,R_channel=None):
        assert os.path.isdir(root)
        assert mode in ['train', 'gallery', 'query']
        ...
        if mode == 'train':
            ...
            self.ids = [int(path.split('/')[-2]) for path in img_paths]
        print('====RGB_Infrared>', self.RGB_to_Infrared)
        print('====R_channel>', self.R_channel)
    def __len__(self):
        return len(self.img_paths)
