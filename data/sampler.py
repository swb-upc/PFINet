...
class CrossModalityRandomSampler(Sampler):
    ...
    def __len__(self):
        print(max(len(self.rgb_list), len(self.ir_list)) * 2)
        return max(len(self.rgb_list), len(self.ir_list)) * 2

    def __iter__(self):
        sample_list = []
        rgb_list = np.random.permutation(self.rgb_list).tolist()  #随机排序，打乱顺序
...
class CrossModalityIdentitySampler(Sampler):
    '''
        每个batch中身份一致，但每个身份只取一次，剩余的图片就不用了
    '''
...
class RandomIdentitySampler(Sampler):
    def __init__(self, data_source, batch_size, num_instances):
        ...
        # estimate number of examples in an epoch(only estimate,not the final)
        self.length = 0
    ...
    def __iter__(self):
        ...
        for pid in self.pids:
            ...
            for idx_I, idx_R in zip(idxs_I, idxs_R):
                batch_idxs.append(idx_I)              #先放IR  [IR,RGB]
                batch_idxs.append(idx_R)              #后放RGB
